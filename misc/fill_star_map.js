...
function*gen_fill(params, sv) {
	...
	var r
	r = yield client.execute('truncate users', [], 1, sv.resume)
	r = yield client.execute('truncate auth_sessions', [], 1, sv.resume)
	r = yield client.execute('truncate buildings', [], 1, sv.resume)
	r = yield client.execute('truncate config', [], 1, sv.resume)
	...
	r = yield client.execute('truncate live_planets', [], 1, sv.resume)

	var gnames = 'Андромеда,Водоворот,Веретено,Центавр'.split(','), nl = gnames.length; while(nl--) {

		var gn = gnames[nl]
			
		var gid = cql.types.uuid() 
		r = yield client.execute("insert into galaxys (galaxy_uuid,name,x,y,z) values (?,?,1,1,1)", [gid, gn], 1, sv.resume)
