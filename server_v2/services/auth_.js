...
var Info = APIRequest.inherit({
	request_gen: function*(params,sv) {
		if(!(config.sessionCookie in params.self.req._cookie)) {
			params.self.res.setHeader('Set-Cookie', config.sessionCookie+'=123; expires=' + (new Date(Date.now()+60*1000*60*24*60)).toUTCString()+'; httponly=true; path=/')
			return { exists: false }
		}
