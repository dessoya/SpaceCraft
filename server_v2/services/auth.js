...
*/
		...
		if(session.is_auth) {
			...
			if(finded_uuid !== null && finded_uuid !== user_uuid) {
				...
				yield Session.update(session_uuid, { auth_method: method }, sv.resume)
			...
			yield Session.update(session_uuid, { is_auth: true, user_uuid: user_uuid, auth_method: method }, sv.resume)
			yield Session.updateLastAccess(session_uuid, sv.resume3end)
		}
